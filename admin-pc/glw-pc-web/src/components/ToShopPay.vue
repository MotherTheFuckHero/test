    ...
    <div class="to-shop-pay-content">
      <div v-if="shopDetailObjPay.productType !== 2 && shopDetailObjPay.productType !== 3" class="to-pay-head">
        ...
        <div class="to-pay-head-content">
          <div :class="{ addressActive : index === isActive }" v-for="(item, index) in filteraddressModelList" :key="index" class="to-pay-head-content-model" @click="checkAddress(index)">
            <h3>{{$t("pay.seven")}}：{{item.receiverName}}<span>{{item.isDefault === 1 ? $t("pay.seven") : ''}}</span></h3>
            <div>
              {{item.receiverStateName}}&nbsp;{{item.receiverCityName}}&nbsp;{{item.receiverDistrictName}}&nbsp;{{item.receiverAddress}}&nbsp;{{item.receiverMobile}}
            </div>
          </div>
        </div>
        <p class="go-general" @click="goGeneral">{{$t("pay.Managereceivingaddress")}}</p>
      </div>
        ...
        <div class="to-pay-head-content" >
          <RadioGroup v-model="payment" >
            <Radio style="margin:10px 20px;display: flex;align-items: center;justify-content: center;float: left" :label="item.code" v-for="item in paymentList" :value="item.code" :key="item.code">
              ...
              <div v-if="item.code=='union_pay'" style="text-align: center;">
      ...
      <div class="to-pay-confirm">
        ...
        <div class="to-pay-confirm-content">
          ...
          <div>
            <p>{{shopSpecListPay === undefined ? shopDetailObjPay.title : shopSpecListPay.productSpecs}}</p>
          </div>
          <div>
            <p>{{shopDetailObjPay.platFormPrice}}</p>
          </div>
          <div>
            <InputNumber :min="1" v-model="shopCountPay" size="small"></InputNumber>
          </div>
          <div>
            <p>{{(shopDetailObjPay.platFormPrice * shopCountPay).toFixed(2)}}</p>
          </div>
        </div>
        <button @click="confirmOrder">{{$t("pay.six")}}</button>
      </div>
    </div>
    <Modal
      v-model="payNative"
      :title="this.$t('qrcode.one')"
      :closable="false"
      :mask-closable="false"
    >
      <div id="qrcode" ref="qrCodeUrl"></div>
      <p class="cp">
        {{$t('qrcode.two')}}<span style="color:red;font-weight: bold;">{{$t("operate.Money")}}{{this.calculation.totalPrice}}</span>
        <span v-if="this.calculation.couponPrice>0">{{$t('qrcode.three')}}<span style="color:red;font-weight: bold;">{{$t("operate.Money")}}{{this.calculation.couponPrice}}</span></span>
        {{$t('qrcode.four')}}<span style="color:red;font-weight: bold;">{{$t("operate.Money")}}{{this.calculation.payPrice}}</span>
      </p>
      <p class="cp">{{$t('qrcode.five')}}</p>
      <div slot="footer">
        <Button size="large" long @click="cancelPay">{{$t('qrcode.six')}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
<script>
    import { getAddressList, getOrderdetail,getcontactsList,getorderpayment,postcouponlist,postcalculation } from '../api/order'
    ...
    export default {
        ...
        created() {
            this.shopDetailObjPay = this.$route.query.shopDetailObj;
            console.log(this.shopDetailObjPay)
            this.shopSpecListPay = this.$route.query.shopSpecList;
            this.shopCountPay = Number(this.$route.query.shopCount);
            let data={
                            ...
                            if(this.calculation.payPrice==0){
                                ...
                                getPay(payData).then(respones => {
                                    if (respones.code === '200'){
                                        window.location.href = respones.data.msg;
                                    }else{
