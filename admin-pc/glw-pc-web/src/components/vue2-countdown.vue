<template>
  <div>
    <p v-if="msTime.show">
      ...
      <span v-if="msTime.day>0"><span>{{msTime.day}}</span><em>{{dayTxt}}</em></span>
      <span>{{msTime.hour}}</span><em>{{hourTxt}}</em>
      <span>{{msTime.minutes}}</span><em>{{minutesTxt}}</em>
      <span>{{msTime.seconds}}</span><em>{{secondsTxt}}</em>
    </p>
