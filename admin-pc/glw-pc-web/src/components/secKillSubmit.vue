    ...
    <div class="to-shop-pay-content">
      ...
      <div class="to-pay-confirm">
        ...
        <div class="to-pay-confirm-content">
          ...
          <div>
            <p>{{shopSpecListPay.productSeckill.secKillPrice}}</p>
          </div>
          <div>
            <p>{{this.shopCountPay}}</p>
          </div>
          <div>
            <p>{{(shopSpecListPay.productSeckill.secKillPrice * shopCountPay).toFixed(2)}}</p>
          </div>
        </div>
    ...
    export default {
        ...
        created() {
          getorderpayment().then(res=>{
           ...
           let secKillData = JSON.parse(localStorage.getItem("secKillData"));
            this.shopDetailObjPay = secKillData.shopDetailObj;
            this.shopSpecListPay = secKillData.shopSpecList;
            this.shopCountPay = Number(secKillData.shopCount);
          let data={
        ...
        methods: {
           ...
           checkcontacts(i) {
            ...
            confirmOrder() {
                ...
                let specRequests = [{
                    price:this.shopSpecListPay.productSeckill.secKillPrice,
                    specId:this.shopSpecListPay.id,
                    productNum: this.shopCountPay,
                    secKillId:this.shopSpecListPay.productSeckill.id,
                }]
                let data = {
