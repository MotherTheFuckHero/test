<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column prop="linkName" label="商家联系人" width="150px" />
      <el-table-column prop="linkPhone" label="商家联系电话" width="150px" />
      ...
      <el-table-column prop="totalReturnCouponAmount" label="优惠券还金额" width="150px" />
      ...
      <el-table-column prop="totalCommunityHeadExpensesAmount" label="总社区团长佣金金额" width="150px" />
      <el-table-column label="创建人" prop="createdUserName" width="200" />
      <el-table-column label="创建时间" prop="createdTime" width="200" />
      <!--   编辑与删除   -->
      <el-table-column
        v-permission="['settlement_info_update','settlement_info_delete']"
        label="操作"
        width="430px"
        align="center"
        fixed="right"
      >
        <template slot-scope="scope">
          <el-button v-permission="['settlement_info_select']" size="mini" type="primary" plain @click="detailsEdit(scope.$index, scope.row)">查看详情</el-button>
          <el-button v-permission="['settlement_info_update']" size="mini" type="success" plain @click="exportDetail(scope.$index, scope.row)">导出结算详情</el-button>
          <el-button v-show="scope.row.status === 3? true:false " v-permission="['settlement_info_update']" size="mini" type="primary" plain @click="handleEdit(scope.$index, scope.row)">修改完毕</el-button>
          <el-button v-show="scope.row.status === 2? true:false " v-permission="['settlement_info_update']" size="mini" type="primary" plain @click="paymentEdit(scope.$index, scope.row)">确认完毕准备打款</el-button>
          <el-button v-show="scope.row.status === 5? true:false " v-permission="['settlement_info_update']" size="mini" type="primary" plain @click="completeEdit(scope.$index, scope.row)">打款完毕</el-button>
          <el-button v-show="scope.row.status === 6? true:false " v-permission="['settlement_info_update']" size="mini" type="primary" plain @click="collectionEdit(scope.$index, scope.row)">商户确认收款</el-button>
          <el-button v-show="scope.row.status !== 10? true:false " v-permission="['settlement_info_update']" size="mini" type="primary" plain @click="closeEdit(scope.$index, scope.row)">关闭</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
    ...
    <el-dialog title="结算详情" :visible.sync="dialogTableVisible">
      <el-table :data="resultList">
        ...
        <el-table-column property="merchantName" label="商家" width="150" />
        ...
        <el-table-column property="returnCouponAmount" label="优惠券返回金额" width="150" />
        ...
        <el-table-column property="communityHeadExpensesAmount" label="社区团长佣金金额" width="200" />
      </el-table>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  cruds() {
    return CRUD({
      ...
      exportUrl: process.env.VUE_APP_cashier_API + 'api/settlement_info/export',
      url: process.env.VUE_APP_cashier_API + 'api/settlement_info/page_list',
      crudMethod: { ...crudJob }
    })
  },
  mixins: [presenter()],
