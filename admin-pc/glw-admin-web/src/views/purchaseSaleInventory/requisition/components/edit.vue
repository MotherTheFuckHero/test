  ...
  <div class="purchaseOrderDetail">
      ...
      <div class="purchaseOrderOneContent">
        <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px" class="demo-addForm">
          <el-form-item label="调出仓库" prop="outWarehouseId">
            <el-select v-model="form.outWarehouseId" @change="getWarehouse" size="mini" placeholder="请选择调出仓库">
              <el-option
          ...
          <el-form-item label="调入仓库" prop="inWarehouseId">
            <el-select v-model="form.inWarehouseId" size="mini" placeholder="请选择调入仓库">
              <el-option
                v-for="item in warehouseData"
                :key="item.id"
                :label="item.name"
    ...
    <div class="purchaseOrderTwo">
        ...
        <el-table ref="table" v-loading="loading" :data="tableData" style="width: 100%;" show-summary :summary-method="getTotal">
          ...
          <el-table-column prop="afterAllotQuantity" label="调拨后数量" align="center" >
            <template slot-scope="scope">
              <el-input type="number" v-model="scope.row.afterAllotQuantity" size="mini" placeholder="请输入商品数量" @input="getTotalPrice(scope.row,scope.$index)"/>
            </template>
          </el-table-column>
          <el-table-column prop="quantity" label="数量" align="center" >
            <template slot-scope="scope">
              {{scope.row.quantity = scope.row.beforeAllotQuantity - scope.row.afterAllotQuantity}}
            </template>
          </el-table-column>
          <el-table-column prop="price" label="单价" align="center" >
            <template slot-scope="scope">
              <el-input type="number" v-model="scope.row.price" size="mini" placeholder="请输入商品单价" @input="getTotalPrice(scope.row,scope.$index)"/>
            </template>
          </el-table-column>
      ...
      <div style="display: flex;justify-content: center;margin-top: 10px">
        <el-button class="filter-item" size="mini" type="warning" @click="selectProduct">选择商品</el-button>
      </div>
    </div>
    <div style="margin-top: 10px">
      <el-button class="filter-item" size="mini" type="primary" @click="onePay">保存</el-button>
    </div>
    <allComponents :interface="2" :warehouseId="outWareHouseId" :type="zjDialog" @dialogType="getDialogType" @productData="getProductData"/>
  </div>
</template>

<script>
    import { getStoreHouse } from '@/api/newInvoicing/storehouse'
    import { getAllotOrderDetail, updateAllotOrder } from '@/api/newInvoicing/requisition'
        ...
        data(){
            return{
                ...
                form: {
                    outWarehouseId: '',
                    inWarehouseId: '',
                    allotDate: new Date().getFullYear() + '-' +
                    ...
                    if (newData){
                        getAllotOrderDetail(newData).then(res => {
                            if (res.code === '0'){
                                ...
                                this.loading = false
                                this.form.allotDate = res.data.allotDate
                                this.outWareHouseId = res.data.outWarehouseId
                                this.form.outWarehouseId = res.data.outWarehouseId
                                this.form.inWarehouseId = res.data.inWarehouseId
                                this.form.remark = res.data.remark
                                this.tableData = res.data.allotItemList
                            }
                        })
                    }
                },
        ...
        methods: {
            ...
            getProductData(data){
                let newData = {
                    ...
                    title: data.title,
                    unit: data.unit,
                    beforeAllotQuantity: data.stock,
                    afterAllotQuantity: '',
                    quantity: 0,
                    price: '',
                    amount: 0,
            ...
            getTotalPrice(v, i) {
                v.amount = 0
                if (v.afterAllotQuantity !== '' && v.price !== '') {
                    v.amount = Number((Number(v.beforeAllotQuantity - v.afterAllotQuantity) * Number(v.price)).toFixed(2))
                } else if (v.afterAllotQuantity === '' || v.price === '') {
                    v.amount = ''
                }
            },
            toDelete(id, data){
                ...
                this.$refs.form.validate((valid)=>{
                    if (valid){
                        ...
                        let data = {
                            ...
                            outWarehouseId: this.form.outWarehouseId,
                            allotDate: this.form.allotDate,
                            ...
                            merchantId: 10022,
                            companyId: 1,
                            allotItemList: this.tableData
                        }
                        updateAllotOrder(this.editingId, data).then(res => {
                            if (res.code === '0'){
