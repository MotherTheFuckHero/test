<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column prop="merchantName" label="导游" width="150px" />
      ...
      <!--   编辑与删除   -->
      <el-table-column
        v-permission="['guide_settlement_info_update','guide_settlement_info_delete']"
        label="操作"
        width="430px"
        align="center"
        fixed="right"
      >
        <template slot-scope="scope">
          <el-button v-permission="['guide_settlement_info_select']" size="mini" type="primary" plain @click="detailsEdit(scope.$index, scope.row)">查看详情</el-button>
          <el-button v-permission="['guide_settlement_info_update']" size="mini" type="success" plain @click="exportDetail(scope.$index, scope.row)">导出结算详情</el-button>
          <el-button v-show="scope.row.status === 3? true:false " v-permission="['guide_settlement_info_update']" size="mini" type="primary" plain @click="handleEdit(scope.$index, scope.row)">修改完毕</el-button>
          <el-button v-show="scope.row.status === 2? true:false " v-permission="['guide_settlement_info_update']" size="mini" type="primary" plain @click="paymentEdit(scope.$index, scope.row)">确认完毕准备打款</el-button>
          <el-button v-show="scope.row.status === 5? true:false " v-permission="['guide_settlement_info_update']" size="mini" type="primary" plain @click="completeEdit(scope.$index, scope.row)">打款完毕</el-button>
          <el-button v-show="scope.row.status === 6? true:false " v-permission="['guide_settlement_info_update']" size="mini" type="primary" plain @click="collectionEdit(scope.$index, scope.row)">商户确认收款</el-button>
          <el-button v-show="scope.row.status !== 10? true:false " v-permission="['guide_settlement_info_update']" size="mini" type="primary" plain @click="closeEdit(scope.$index, scope.row)">关闭</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
    ...
    <el-dialog title="结算详情" :visible.sync="dialogTableVisible">
      <el-table :data="resultList">
        ...
        <el-table-column property="merchantName" label="导游" width="150" />
...
export default {
  ...
  cruds() {
    return CRUD({
      ...
      exportUrl: process.env.VUE_APP_cashier_API + 'api/settlement_info/export_guide',
      url: process.env.VUE_APP_cashier_API + 'api/settlement_info/guide_page_list',
      crudMethod: { ...crudJob }
    })
  },
  mixins: [presenter()],
  ...
  data() {
    return {
      ...
      permission: {
        add: ['guide_settlement_info_add'],
        edit: ['guide_settlement_info_update'],
        del: ['guide_settlement_info_delete'],
        export: ['guide_settlement_info_export']
      }
    }
  },
  methods: {
