<template>
  <div class="lossOrder-index">
    <div class="lossOrder-header">
      <div>
      ...
      <div>
        <el-button class="filter-item" size="mini" type="primary" icon="el-icon-plus" @click="showPurchaseOrder = true">创建损益单</el-button>
      </div>
    </div>
    <div class="lossOrder-body">
      <el-table ref="table" v-loading="loading" :data="tableData" style="width: 100%;">
        <el-table-column type="index" label="#" />
        <el-table-column prop="code" label="订单编号" />
        <el-table-column prop="indecDate" label="损益日期"/>
        <el-table-column prop="statusDesc" label="是否入库"/>
...
<script>
    import { getAllLossOrder, lossOrderWarehousing, delLossOrder } from '@/api/newInvoicing/lossOrder'
    ...
    export default {
        ...
        methods: {
            ...
            getTableData(data, no, size){
                getAllLossOrder(data, no, size).then(res => {
                    if (res.code === '0'){
                        this.loading = false
            ...
            warehousing(id){
                lossOrderWarehousing(id, {status: 2}).then(res => {
                    if (res.code === '0'){
                        this.getTableData({}, 1, 15)
                        this.$notify({
                            title: '成功',
            ...
            delPurchaseOrder(id){
                this.$confirm('此操作将永久删除该损益单, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    delLossOrder(id).then(res => {
                        if (res.code === '0'){
                            this.getTableData({}, 1, 15)
                            this.$message({
                                type: 'success',
...
<style scoped>
  ...
  .lossOrder-header>div:nth-of-type(2){
    display: flex;
    justify-content: space-around;
    align-items: center;
  }
  .lossOrder-header>div:nth-of-type(2)>div{
    margin-left: 10px;
  }
  .lossOrder-header>div:nth-of-type(2)>button{
    margin-left: 10px;
  }
  .lossOrder-body{
    padding-top: 10px;
