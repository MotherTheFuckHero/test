    ...
    <div class="purchaseOrderDetail">
      <div class="purchaseOrderOne">
        ...
        <div class="purchaseOrderOneContent">
          <h2 style="text-align: center">损益单</h2>
          ...
          <div style="margin-top: 10px">
            <el-table ref="table" v-loading="loading" :data="tableDataOne" style="width: 100%;" show-summary :summary-method="getTotal">
              ...
              <el-table-column prop="actualStock" label="账面库存" />
              <el-table-column prop="stock" label="实际库存" />
              <el-table-column prop="indecQuantity" label="损益数量" />
              <el-table-column prop="price" label="损益单价" />
...
<script>
    import { getLossOrderDetail } from '@/api/newInvoicing/lossOrder'
    export default {
        props: ['getId'],
        data(){
            return{
        ...
        watch: {
            getId: {
                handler(newData){
                    if (newData){
                        getLossOrderDetail(newData).then(res => {
                            if (res.code === '0'){
                                this.loading = false
                                this.detail.code = res.data.code
                                this.detail.date = res.data.indecDate
                                this.tableDataOne = res.data.indecItemList
                            }
                        })
                    }
                },
