...
<script>
    import { getLossOrderDetail, updateLossOrder } from '@/api/newInvoicing/lossOrder'
    ...
    export default {
        ...
        data(){
            return{
                ...
                rules: {
                    indecDate: [
                        {  required: true, message: '请选择损益日期', trigger: 'change' }
                    ],
                },
                editingId: null
            }
                    ...
                    if (newData){
                        getLossOrderDetail(newData).then(res => {
                            if (res.code === '0'){
                                ...
                                this.loading = false
                                this.form.indecDate = res.data.indecDate
                                this.form.remark = res.data.remark
                                this.tableData = res.data.indecItemList
                            }
                        })
                    }
                },
            ...
            towPay(){
                this.$refs.form.validate((valid)=>{
                    if (valid){
                        ...
                        updateLossOrder(this.editingId, data).then(res => {
                            if (res.code === '0'){
