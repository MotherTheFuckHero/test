  ...
  <div class="purchaseorder-index">
    <div class="purchaseorder-header">
      ...
      <div>
        ...
        <el-button class="filter-item" size="mini" type="primary" icon="el-icon-plus" @click="showPurchaseOrder = true">创建采购单</el-button>
      </div>
    </div>
    <div class="purchaseorder-body">
      <el-table ref="table" v-loading="loading" :data="tableData" style="width: 100%;">
        ...
        <el-table-column prop="purchaseDate" label="采购日期" width="200px"/>
        <el-table-column prop="payStatusDesc" label="付款状态" width="200px"/>
        <el-table-column prop="totalAmount" label="总价" width="200px"/>
        <el-table-column prop="payAmount" label="已付金额" width="200px"/>
        <el-table-column prop="statusDesc" label="是否入库" width="200px"/>
        <el-table-column prop="remark" label="备注" width="200px"/>
        <el-table-column label="操作" width="480px" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button v-if="scope.row.status == 1&&scope.row.payStatus == 2" size="mini" type="primary" icon="el-icon-check" @click="paying(scope.row.id)">付款</el-button>
            <el-button v-if="scope.row.status == 1" size="mini" type="success" icon="el-icon-finished" @click="warehousing(scope.row.id)">入库</el-button>
...
<script>
    import { getAllPurchaseOrder, purchaseOrderWarehousing, delPurchaseOrder, payPurchaseOrder } from '@/api/newInvoicing/purchaseorder'
    ...
    export default {
        ...
        methods: {
            ...
            getTableData(data, no, size){
                getAllPurchaseOrder(data, no, size).then(res => {
                    if (res.code === '0'){
                        this.loading = false
            ...
            warehousing(id){
                purchaseOrderWarehousing(id, {status: 2}).then(res => {
                    if (res.code === '0'){
                        this.getTableData({}, 1, 15)
                        this.$notify({
                            title: '成功',
            ...
            delPurchaseOrder(id){
                this.$confirm('此操作将永久删除该进货单, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    delPurchaseOrder(id).then(res => {
                        if (res.code === '0'){
                            this.getTableData({}, 1, 15)
                            this.$message({
                                type: 'success',
...
.purchaseorder-header>div:nth-of-type(2){
  display: flex;
  justify-content: space-around;
  align-items: center;
}
.purchaseorder-header>div:nth-of-type(2)>div{
  margin-left: 10px;
}
.purchaseorder-header>div:nth-of-type(2)>button{
  margin-left: 10px;
}
.purchaseorder-body{
  padding-top: 10px;
