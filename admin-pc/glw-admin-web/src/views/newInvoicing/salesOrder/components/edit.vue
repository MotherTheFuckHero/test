      ...
      <div class="purchaseOrderTwoContent">
        <el-table ref="table" v-loading="loading" :data="tableData" style="width: 100%;" show-summary :summary-method="getTotal">
          ...
          <el-table-column prop="salePrice" label="售价" align="center" >
            <template slot-scope="scope">
              <el-input type="number" v-model="scope.row.salePrice" size="mini" placeholder="请输入商品进货价" @input="getTotalPrice(scope.row,scope.$index)"/>
            </template>
          </el-table-column>
...
<script>
    import { getSaleOrderDetail, updateSaleOrder } from '@/api/newInvoicing/salesorder'
    ...
    export default {
        ...
        watch: {
            ...
            editId: {
                handler(newData){
                    if (newData){
                        getSaleOrderDetail(newData).then(res => {
                            if (res.code === '0'){
                                this.editingId = newData
                                this.loading = false
                                this.form.saleDate = res.data.saleDate
                                this.form.remark = res.data.remark
                                this.tableData = res.data.saleItemList
                            }
                        })
                    }
                },
            ...
            towPay(){
                this.$refs.form.validate((valid)=>{
                    if (valid){
                        ...
                        updateSaleOrder(this.editingId, data).then(res => {
                            if (res.code === '0'){
