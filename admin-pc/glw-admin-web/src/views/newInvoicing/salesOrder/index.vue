  ...
  <div class="salesOrder-index">
    <div class="salesOrder-header">
      ...
      <div>
        ...
        <el-button class="filter-item" size="mini" type="primary" icon="el-icon-plus" @click="showPurchaseOrder = true">创建销售单</el-button>
      </div>
    </div>
    <div class="salesOrder-body">
      <el-table ref="table" v-loading="loading" :data="tableData" style="width: 100%;">
        ...
        <el-table-column prop="saleDate" label="销售日期" width="200px"/>
        ...
        <el-table-column label="操作" width="480px" align="center" fixed="right">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.status == 1" size="mini" type="success" icon="el-icon-finished" @click="warehousing(scope.row.id)">出库</el-button>
...
<script>
    import { getAllSalesOrder, delSalesOrder, salesOrderSale, paySaleOrder } from '@/api/newInvoicing/salesorder'
    ...
    export default {
        ...
        methods: {
            ...
            getTableData(data, no, size){
                getAllSalesOrder(data, no, size).then(res => {
                    if (res.code === '0'){
                        this.loading = false
            ...
            warehousing(id){
                salesOrderSale(id, {status: 2}).then(res => {
                    if (res.code === '0'){
                        this.getTableData({}, 1, 15)
                        this.$notify({
                            title: '成功',
...
<style scoped>
  ...
  .salesOrder-header>div:nth-of-type(2){
    display: flex;
    justify-content: space-around;
    align-items: center;
  }
  .salesOrder-header>div:nth-of-type(2)>div{
    margin-left: 10px;
  }
  .salesOrder-header>div:nth-of-type(2)>button{
    margin-left: 10px;
  }
  .salesOrder-body{
    padding-top: 10px;
