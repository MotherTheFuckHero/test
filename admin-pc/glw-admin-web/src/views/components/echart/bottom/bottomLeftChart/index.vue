    ...
    getbigData(type).then(res => {
      ...
      if (code === '0') {
        for (var i = 0; i < res.data.length; i++) {
          ...
          this.cdata.lineData.push(res.data[i].resValue) // 退单量
          this.cdata.barData.push(res.data[i].value) // 订单量
        }
        this.setData()
      }
    })
  ...
  methods: {
    ...
    setData() {
      for (let i = 0; i < this.cdata.barData.length; i++) {
        const rate = (this.cdata.barData[i] - this.cdata.lineData[i]) / 1000
